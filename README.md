...
在鸿蒙设备上运行完整的 AI Agent，支持 ReAct 推理循环、工具调用、持久化记忆、定时任务、MCP 远程工具和 RAG 知识检索。
...
- `siliconflow_api_key`：SiliconFlow API 密钥（RAG 向量检索用，免费）
- `siliconflow_api_url`：Embedding API 地址（默认 `https://api.siliconflow.cn/v1/embeddings`）
- `embedding_model`：Embedding 模型（默认 `BAAI/bge-m3`）
- `mcp_servers`：MCP Server 列表（JSON 数组，默认高德地图）
...
数据驱动的斜杠命令扩展机制。每个 Skill 是一个 JSON 文件，定义命令名和 prompt 模板。

- 用户输入 `/translate 你好` → 匹配 Skill → 展开 prompt 模板 → 发给 LLM
- `/help` 直接返回帮助文本，不调 LLM
- 存 DB 用原始输入，发 LLM 用展开后的 prompt
- 扩展：往 `rawfile/skills/` 丢 JSON 文件 + 注册到 `skills_index.json`，不改代码

详见 [skill-impl.md](skill-impl.md)。

### 8. MCP Client — `services/McpClient.ets`

标准 MCP（Model Context Protocol）客户端，Streamable HTTP Transport，连接外部 MCP Server 动态获取工具。

- JSON-RPC 2.0 握手（initialize → notifications/initialized → tools/list）
- 远程工具自动转换为 OpenAI function calling 格式，与本地工具合并
- Session 失效自动重连
- 默认配置高德地图 MCP Server（15 个地理工具）

详见 [mcp-impl.md](mcp-impl.md)。

### 9. RAG 检索 — `services/VectorRagService.ets`

端侧 RAG（Retrieval-Augmented Generation），让 LLM 基于本地知识库回答问题。

- **向量检索（主路径）**：SiliconFlow BGE-M3 算 embedding（1024 维），SQLite 存向量，余弦相似度取 Top-3
- **关键词匹配（后备）**：API 不可用时自动降级，纯 CPU 计算
- 知识库随 HAP 包分发（`rawfile/knowledge/`），扩展只需加 `.txt` 文件 + 注册到 `kb_index.json`
- 首次启动 ~1-2 秒建索引，后续启动毫秒级就绪

详见 [rag-impl.md](rag-impl.md)。
...
- **Embedding API**：SiliconFlow BGE-M3（免费，用于 RAG 向量检索）
- **MCP 协议**：Streamable HTTP Transport，JSON-RPC 2.0
...
2. 连接鸿蒙设备或启动模拟器
3. Build & Run
4. 开箱即用（API Key 已预置默认值）
5. 如需修改配置，进入 Settings 页面
...
```
你：北京今天天气怎么样
Agent：[调用 maps_weather via 高德地图] → 返回天气数据
Agent：北京今天晴，气温 12-22°C...

你：从西二旗到国贸怎么走
Agent：[调用 maps_direction_transit_integrated] → 返回路线规划
```

验证 RAG（知识检索）：
```
你：怎么连蓝牙
Agent：（基于知识库检索）进入"设置 > 蓝牙"，将蓝牙开关切换为开启状态...

你：OpenHarmony 6.0 有什么新特性
Agent：（基于知识库检索）OpenHarmony 6.0 全面采用 ArkTS 作为主力开发语言...
```
